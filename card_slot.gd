	...
	if is_ace_slot:
		...
		if is_empty():
			if _card.value == 1:
				...
				cards.append(_card)
				#_card.rect_global_position = position
				#_card.set_state(_card.State.IDLE, _card.current_state)
				if by_doubleclick:
		...
		if is_empty():
			...
			if can_place(_card, last_card):
				...
				if last_card != _card:
					last_card.lower_card = _card
				#_card.rect_global_position = last_card.rect_global_position + Vector2(0, Globals.OFFSET)
				#_card.set_state(_card.State.IDLE, _card.current_state)
				_card.is_in_play = true
